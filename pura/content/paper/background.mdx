...
Catalini et al. (Catalini et al., 2026) model the AGI transition as a collision between two cost curves: an exponentially decaying cost to automate ($c_A$), driven by compute and accumulated knowledge, and a biologically bottlenecked cost to verify ($c_H$), bounded by human time and cognitive throughput. A measurability parameter $m$ determines which tasks cross the automation frontier: when $m$ is high, output quality can be checked cheaply; when $m$ is low, verification requires expensive human judgment. The widening gap $\Delta_m = m_A - m_H$ between what agents can execute and what humans can afford to verify is, in their framework, the binding constraint on realized economic value. BPE's `CompletionTracker` is a concrete implementation of the verification function $V(m)$ for high-$m$ tasks: dual-signed completion receipts, rolling-window epoch accounting, and auto-slashing produce an on-chain verification signal at sub-cent marginal cost. For low-$m$ tasks where completion receipts are insufficient, the `verificationBudgetBps` parameter lets economy deployers earmark a fraction of payment streams for out-of-band verification. Catalini and Gans (Catalini & Gans, 2020) separately argue that cryptographic provenance and stablecoin settlement reduce verification transaction costs by an order of magnitude, which is the same architectural thesis behind BPE's deployment on Superfluid GDA with Base L2 settlement.
