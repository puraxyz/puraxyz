...
The Pura gateway is an OpenAI-compatible API that routes across four LLM providers (OpenAI, Anthropic, Groq, Gemini). It picks the cheapest model that fits your task and tracks per-request costs.
...
Pura picks the model automatically. Simple questions go to Groq or Gemini (cheapest). Complex reasoning goes to Anthropic or OpenAI.
...
On-chain capacity weights (GDA pool units on Base Sepolia) break ties between providers in the same tier.
