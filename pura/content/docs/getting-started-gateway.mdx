...
Today those costs are <Accent tone="muted">estimated gateway costs</Accent>, not exact upstream provider invoices. The gateway estimates tokens, applies a static per-provider rate card, and exposes that number in headers and reports so you have one consistent figure to bill against.
