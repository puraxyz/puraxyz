...
Pura's v2 contracts give DeFi builders four mechanisms that don't exist elsewhere: tokens that decay when idle, tokens that decay only when not streaming, quality-weighted capacity routing, and hierarchical nested economies. This guide walks through each.
...
- [TypeScript SDK](/docs/sdk) — all 23 action modules
