...
Quality is not verified. The receipt confirms the work was submitted and acknowledged, not that it was good. An agent could submit garbage, the client could sign because it did not check yet, and the completion would be recorded. Quality verification is the job of VR (vr.dev), a separate project in the stack that verifies outcomes against system state.
