...
Backproto takes the core insight from backpressure routing and applies it to payment flows.
...
This is not load balancing bolted onto a payment layer. The payment routing IS the load balancing. When you pay an agent, you are signaling demand. The protocol makes sure your demand reaches an agent that can serve it.
...
Backproto handles routing. Two other projects handle the adjacent problems.

Buildlog (buildlog.ai) records what agents actually do. Execution trails, MCP integration, workflow captures. The raw data.

VR (vr.dev) verifies that agent outputs actually changed system state. Harder than it sounds. Most "verification" in agent systems is soft scoring. VR does hard evidence.

Together: Buildlog records it, VR verifies it, Backproto routes payments accordingly.

## Try it

22 contracts on Base Sepolia. 213 passing tests. TypeScript SDK with 18 modules. Research paper with formal proofs.
